...
utils.urlify = function (url) {
  return url.toString().toLowerCase().replace(/\//g, ".");
};

utils.getDocletParent = function (doc, doclets) {
  return doclets
    .filter((d) => {
      return d.id === doc.parent && d.metadata.index < doc.metadata.index;
    })
    .pop();
};
...
utils.isUrl = function (url) {
  url = (url + "").trim(); //clean url input
  if (url.indexOf("./") === 0) return true; //absolute url (root relative)
  if (url.indexOf("/") === 0) return true; //relative url (file relative)
  if (url.indexOf("$") === 0) return true; //custom relative url (settings path)
  return false;
};

/**
 * Resolve final value for the give url string.
 * @method resolveUrl
 * @param {String} url
 * @param {String} file
 * @param {Object} opts
...
utils.resolveUrl = function (url, file, opts) {
  ...
  if (url.indexOf("./") === 0) return opts.rootPath + url.substr(2);
  ...
  if (url.indexOf("$") === 0 && opts.customPaths) {
    ...
    if (customPath) return customPath + parts.join("/");
  }
 ...
 * @method findGlobFiles
 * @param {String | Array<String>} patterns
 * @return {Array<String>}
 */
utils.findGlobFiles = function (patterns) {
  var result = [],
    match = [],
    ignore = [];
  ...
  patterns.forEach(function (value) {
    if (!value) return;
    if (value.indexOf("!") === 0) ignore.push(value.substr(1));
    else match.push(value);
  });
 ...
 * @returns {Promise}
 */
utils.readFiles = function (files, callback) {
